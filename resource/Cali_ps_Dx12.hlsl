...
static const float3 SEA_BASE = float3(0.11,0.19,0.22);
static const float3 SEA_WATER_COLOR = float3( 0.55,0.9,0.7);
#define SEA_TIME (iTime * SEA_SPEED)

//Matrix to permute the water surface into a complex, realistic form
//static const float2x2 octave_m = float2x2(1.7,1.2,-1.2,1.4); // OLD
...
static const float KEY_SP    = 32.5/256.0;
...
float map_detailed(float3 p) {
    ...
    for(int i = 0; i < ITER_FRAGMENT; i++) {
        ...
        // This is likely the most important parameter for wave topology.
    	//uv *= octave_m/1.2; // OLD
		uv = mul(uv, octave_m / 1.2);
		//uv = mul( octave_m/1.2, uv );
		//uv = mul(uv, transpose( octave_m / 1.2 ) );
		//uv = mul( uv, octave_m/0.9 );
...
float3 getSeaColor(float3 p, float3 n, float3 l, float3 eye, float3 dist) {  
    ...
    //color += float3(specular(n,l,eye,90.0))*0.5; // OLD
	float specular_ = specular( n,l,eye,90.0 );
    color += float3( specular_, specular_, specular_ ) * 0.5;
...
{
	//return texture(iChannel1, float2(key, 1.0)).x; // OLD
	return false; // no night mode
	//return true; // night mode
}
...
float4 main(float4 fragCoord : SV_POSITION) : SV_TARGET { // psh
   ...
   //  dir.z += length(uv) * 0.15;
	...
	//dir = mul( normalize(dir), fromEuler( ang ) ); // OLD
